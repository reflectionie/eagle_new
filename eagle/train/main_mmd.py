...
# 新增两个参数，用于控制 MMD loss 的权重
parser.add_argument('--w1', type=float, default=0.0, help="权重1：用于隐藏状态的 MMD loss")
parser.add_argument('--w2', type=float, default=0.0, help="权重2：用于概率的 MMD loss")
# 新增参数：用于恢复训练的检查点路径和起始 epoch
parser.add_argument('--resume_checkpoint', type=str, default=None, help="恢复训练的检查点目录")
parser.add_argument('--resume_epoch', type=int, default=0, help="从哪个 epoch 开始继续训练")
...
if accelerator.is_main_process:
    ...
    args.wandb_run_name = str(args.wandb_run_name) + "eagel_new" + run_name
...
def compute_loss(target, target_p, predict, loss_mask):
    ...
    mask = loss_mask.squeeze(-1) > 0
    if mask.sum() > 0:
        pred_features = predict[mask]
        target_features = target[mask]
        mmd_feature = mmd_loss(pred_features, target_features)
        predicted_prob = nn.Softmax(dim=2)(out_head)
        pred_prob = predicted_prob[mask]
        target_prob = target_p[mask]
        mmd_prob = mmd_loss(pred_prob, target_prob)
    else:
        mmd_feature = torch.tensor(0.0, device=predict.device)
        mmd_prob = torch.tensor(0.0, device=predict.device)
        
    return vloss, ploss, out_head, mmd_feature, mmd_prob
...
# 如果传入了 resume_checkpoint 参数，则加载检查点状态，并设置起始 epoch
...
for epoch in range(start_epoch, num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        with accelerator.accumulate(model):
            ...
            vloss, ploss, out_head, mmd_feature, mmd_prob = compute_loss(data["target"], target_p, predict, loss_mask)
            loss = train_config["v_w"] * vloss + args.w1 * mmd_feature + train_config["p_w"] * ploss + args.w2 * mmd_prob
        ...
        if accelerator.is_main_process and ct != 0:
            base_loss = train_config["v_w"] * vloss + train_config["p_w"] * ploss
            mmd_loss_value = args.w1 * mmd_feature + args.w2 * mmd_prob
            total_loss = base_loss + mmd_loss_value
            logdict = {
                "train/lr": optimizer.optimizer.param_groups[0]["lr"],
                "train/vloss": vloss.item(),
                "train/ploss": ploss.item(),
                "train/loss": base_loss.item(),
                "train/total_loss": total_loss.item(),
                "train/mmd_loss": mmd_loss_value.item(),
                "train/mmd_feature": mmd_feature.item(),
                "train/mmd_prob": mmd_prob.item(),
                "train/acc": cc / ct
            }
    ...
    if (epoch + 1) % train_config["save_freq"] == 0:
        ...
        for batch_idx, data in enumerate(tqdm(test_loader)):
            ...
            epoch_mmd_loss += mmd_loss_value.item()
            epoch_mmd_feature += mmd_feature.item()
            epoch_mmd_prob += mmd_prob.item()
            num_batches += 1

        epoch_loss /= num_batches
        epoch_total_loss /= num_batches
        epoch_mmd_loss /= num_batches
        epoch_mmd_feature /= num_batches
        epoch_mmd_prob /= num_batches
        ...
        if accelerator.is_main_process:
            logdict = {
                ...
                "test/epoch_total_loss": epoch_total_loss,
                "test/epoch_mmd_loss": epoch_mmd_loss,
                "test/epoch_mmd_feature": epoch_mmd_feature,
                "test/epoch_mmd_prob": epoch_mmd_prob,
                "test/epoch_acc": correct / total
            }
            for id, i in enumerate(top_3acc):
                logdict[f'test/top_{id + 1}_acc'] = i.sum().item() / total
            wandb.log(logdict)
        ...
        if accelerator.is_main_process:
            ...
            accelerator.save_state(output_dir=f"{run_name}_{args.cpdir}/state_{epoch}")
