...
def loose_topk_condition(topk_pred: torch.Tensor,
    ...
    - intersect_sizes: 长度为 bs 的列表，每个元素为对应样本的交集大小
    """
    ...
    for i in range(batch_size):
        ...
        inter_size = len(set_pred.intersection(set_gt))
        intersect_sizes.append(inter_size)
        if inter_size >= k_sub:
            condition[i] = True
    
    return condition, intersect_sizes
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        with accelerator.accumulate(model):
            ...
            batch_size, seq_len, _ = predict_init.shape

            # 如果决策方法为 topk_loose，则初始化一个用于存储每个序列所有时间步的交集大小的列表
            if train_config['decision_method'] == "topk_loose":
                # 例如：all_intersect_sizes[i] 为第 i 个样本在各个时间步的交集大小列表
                all_intersect_sizes = [[] for _ in range(batch_size)]
            ...
            for t in range(seq_len - 1, 0, -1):
                if train_config['decision_method'] == "topk":
                    ...
                    topk_pred = torch.topk(probs_t, train_config['k'], dim=-1).indices   # [bs, k]
                    topk_gt = torch.topk(target_p[:, t, :], train_config['k'], dim=-1).indices  # [bs, k]
                    # 使用松散判定函数，返回 condition 与当前时间步每个样本的交集大小
                    condition, current_intersect_sizes = loose_topk_condition(topk_pred, topk_gt, train_config['k_sub'])
                    # 将当前时间步的交集大小添加到对应每个样本的列表中
                    for i in range(batch_size):
                        all_intersect_sizes[i].append(current_intersect_sizes[i])
            ...
            # 如果使用的是 topk_loose 模式，则将所有序列的交集大小写入 JSON 文件
            if train_config['decision_method'] == "topk_loose":
                # 从 args.configpath 中提取模型名称，即去除路径中所有 '/' 后的最后一个字段
                model_name = args.configpath.split("/")[-1].split(".")[-2]
                filename = f"{train_config['k']}_top_k_loose_{model_name}.jsonl"
                
                # 将结果组织为一个字典，键为序列索引，值为对应的交集大小列表
                result_dict = {f"seq_{i}": all_intersect_sizes[i] for i in range(batch_size)}
